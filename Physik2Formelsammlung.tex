...
\subsection{\(\frac{g}{cm^3} \rightarrow \frac{kg}{m^3}\)}

Beispiel:

\(7.87 \frac{g}{cm^3} \quad = \quad 7870 \frac{kg}{m^3}\)\\[1em]
\(\frac{g}{cm^3} = \frac{1*10^{-3} kg}{1*10^{-6}m^3}\)
...
\begin{center}
  ...
  \begin{tabular}{l | l | r}
    ...
    \(\Lambda\) & Logarithmisches Dekrement & Einheitenlos\\ \hline
  \end{tabular}
\end{center}
...
\paragraph{Beschleunigung-Zeit-Gesetz} \dotfill \(a(t) = -x_0 * \omega_0^2 * \cos (\omega_0 t + \varphi_0)\)
...
\paragraph{Maximale Beschleunigung} \dotfill \(a_{\max} = x_m * \omega_0^2\)
...
\paragraph{Schwingungsdgl am U-Rohr} \dotfill \(\underbrace{\rho*A*l}_\text{m}*\ddot x + b * \dot x +\underbrace{\rho*A*g*2}_\text{k}x=F_E\)\\
(\(x=x_0*\cos(\omega_0*t)\))

\subsubsection{Fadenpendel}

\paragraph{Schwingungsdauer beim Fadenpendel} \dotfill \(T = 2\pi \sqrt{\frac{l}{g}}\)
\paragraph{Winkelgeschwindigkeit} \dotfill \(\omega = \sqrt{\frac{l}{g}}\)
...
\paragraph{Kinetische Energie} \dotfill \(E_{\text{kin}} = \frac{1}{2} m * v^2\)
\paragraph{Elastische Energie} \dotfill \(E_{\text{el}} = \frac{1}{2} k_{\text{ges}} * x^2\)
...
\paragraph{Zentrifugalbeschleunigung} \dotfill \(a = r \cdot \omega^2\)
...
\myhspace \textbf{\(k \in 0, 1, 2, \dots\)} \textbf{Achtung:} \(n\) entspricht \(k + 1\)!\\
...
\myhspace \textbf{Gilt nur für Grundschwingung!}
\paragraph{Grundschwingung/Wellenlänge gleiche Enden} \dotfill \(f = \frac{c}{2*l} ; \lambda = 2 * l\)
\paragraph{Grundschwingung ungleiche Enden} \dotfill \(f = \frac{c}{4*l} ; \lambda = 4 * l\)
